...
class NotificationConfig:
    ...
    def __init__(self):
        # Basic settings
        self.enabled = os.getenv('EQUINOVA_NOTIFICATIONS', 'true').lower() == 'true'
        self.silent_mode = os.getenv('EQUINOVA_SILENT_MODE', 'false').lower() == 'true'
        self.debug_notifications = os.getenv('EQUINOVA_DEBUG_NOTIFICATIONS', 'false').lower() == 'true'

        # Rate limiting settings
        self.rate_limit_enabled = True
        self.max_notifications_per_minute = int(os.getenv('EQUINOVA_MAX_NOTIFICATIONS_PER_MIN', '5'))
        self.duplicate_suppression_window = int(os.getenv('EQUINOVA_DUPLICATE_WINDOW', '30'))  # seconds

        # Notification levels to show (can be configured via environment)
        enabled_levels = os.getenv('EQUINOVA_NOTIFICATION_LEVELS', 'info,success,warning,error,critical')
        self.enabled_levels = set(level.strip().lower() for level in enabled_levels.split(','))

        # App settings
        self.app_name = os.getenv('EQUINOVA_APP_NAME', 'Finance Terminal')
    ...
    def _get_app_icon(self) -> Optional[str]:
        ...
        icon_path = os.getenv('EQUINOVA_ICON_PATH')
