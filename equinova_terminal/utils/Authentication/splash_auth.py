...
from equinova_terminal.utils.Logging.logger import logger, operation, monitor_performance
            ...
            with operation("show_api_error_screen"):
                ...
                with dpg.child_window(width=460, height=350, border=True, parent=parent):
                    ...
                    with dpg.group(horizontal=True):
                        ...
                        dpg.add_text("Cannot connect to EquiNova API server", color=[255, 150, 150])
    ...
    def show_splash(self) -> Dict[str, Any]:
        ...
        try:
            with operation("show_splash"):
                ...
                with dpg.window(tag="splash_window", label="EquiNova Authentication",
                ...
                title = "EquiNova - Welcome!" if self.is_first_time_user else "EquiNova - Authentication"
        ...
        try:
            with operation("create_welcome_screen"):
                ...
                with dpg.group(horizontal=True, parent=parent):
                    ...
                    dpg.add_text("🚀 EQUINOVA", color=[255, 215, 0])
                ...
                if self.is_first_time_user:
                    with dpg.group(horizontal=True, parent=parent):
                        ...
                        dpg.add_text("👋 Welcome to EquiNova!", color=[100, 255, 100])
                else:
                    with dpg.group(horizontal=True, parent=parent):
                        dpg.add_spacer(width=120)
    ...
    def create_guest_card(self, parent, emphasized=False):
        ...
        try:
            ...
            with dpg.child_window(width=460, height=100, border=True, parent=parent):
                ...
                with dpg.group(horizontal=True):
                    dpg.add_spacer(width=15)
                    text = "🚀 Try EquiNova instantly! No signup required" if self.is_first_time_user else "50 requests/day with temporary API key"
                    dpg.add_text(text, color=[200, 200, 200])
                dpg.add_spacer(height=10)
                with dpg.group(horizontal=True):
    ...
    def create_signup_card(self, parent, emphasized=False):
        ...
        try:
            ...
            with dpg.child_window(width=460, height=100, border=True, parent=parent):
                ...
                with dpg.group(horizontal=True):
                    dpg.add_spacer(width=15)
                    text = "🎁 Join EquiNova for unlimited access"
                    dpg.add_text(text, color=[200, 200, 200])
                dpg.add_spacer(height=10)
                with dpg.group(horizontal=True):
    ...
    def create_guest_session(self):
        ...
        try:
            with operation("create_guest_session"):
                ...
                from equinova_terminal.utils.APIClient.api_client import EquiNovaAPIClient

                temp_session = {"user_type": "guest", "device_id": device_id}
                api_client = EquiNovaAPIClient(temp_session)

                # Use optimized session creation
                result = api_client.get_or_create_guest_session(
                    device_id=device_id,
                    device_name=f"EquiNova - {platform.system()}",
                    platform="desktop",
