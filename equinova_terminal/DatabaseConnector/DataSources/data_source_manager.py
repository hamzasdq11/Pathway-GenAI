...
from equinova_terminal.utils.Logging.logger import info, debug, warning, error, operation, monitor_performance, logger
...
class DataSourceManager:
    ...
    def _get_provider_instance(self, provider_name: str, credentials: Dict[str, str] = None):
        ...
        if provider_name == "alpha_vantage_data":
            ...
            if api_key and len(api_key) > 5:  # Basic validation
                try:
                    from equinova_terminal.DatabaseConnector.DataSources.alpha_vantage_data.alpha_vantage_provider import \
    ...
    def _get_fincept_stock_data(self, symbol: str, period: str, interval: str) -> Dict[str, Any]:
        """Get stock data from EquiNova API (dummy implementation)"""
        logger.debug("Using dummy EquiNova API for stock data", context={"symbol": symbol})
        return {
            "success": True,
            "source": "fincept_api",
            "symbol": symbol,
            ...
            "current_price": 102.0,
            "fetched_at": datetime.now().isoformat(),
            "note": "This is dummy EquiNova API data"
        }

    def _get_fincept_forex_data(self, pair: str, period: str) -> Dict[str, Any]:
        """Get forex data from EquiNova API (dummy implementation)"""
        logger.debug("Using dummy EquiNova API for forex data", context={"pair": pair})
        return {
            "success": True,
            "source": "fincept_api",
            "pair": pair,
            ...
            "current_rate": 1.2345,
            "fetched_at": datetime.now().isoformat(),
            "note": "This is dummy EquiNova forex data"
        }

    def _get_fincept_news_data(self, category: str, limit: int) -> Dict[str, Any]:
        """Get news data from EquiNova API (dummy implementation)"""
        logger.debug("Using dummy EquiNova API for news data", context={"category": category, "limit": limit})
        return {
            "success": True,
            "source": "fincept_api",
            "category": category,
            "articles": [
                {
                    ...
                    "source": "EquiNova News"
                }
            ],
            "total": limit,
            "fetched_at": datetime.now().isoformat(),
            "note": "This is dummy EquiNova news data"
        }
    ...
    def _get_fincept_crypto_data(self, symbol: str, period: str) -> Dict[str, Any]:
        """Get crypto data from EquiNova API (dummy implementation)"""
        logger.debug("Using dummy EquiNova API for crypto data", context={"symbol": symbol})
        return {
            "success": True,
            "source": "fincept_api",
            "symbol": symbol,
            ...
            "fetched_at": datetime.now().isoformat(),
            "note": "This is dummy EquiNova crypto data"
        }

    def _get_fincept_economic_data(self, indicator: str, country: str) -> Dict[str, Any]:
        """Get economic data from EquiNova API (dummy implementation)"""
        logger.debug("Using dummy EquiNova API for economic data",
                     context={"indicator": indicator, "country": country})
        return {
            "success": True,
            "source": "fincept_api",
            ...
            "current_value": 2.5,
            "fetched_at": datetime.now().isoformat(),
            "note": "This is dummy EquiNova economic data"
        }
        ...
        try:
            with operation(f"Test data source {source_name}"):
                ...
                if source_name == "yfinance":
                    ...
                    return {
                        "success": True,
                        "message": "EquiNova API connection successful (dummy)",
                        "response_time": "< 1s"
                    }
                else:
