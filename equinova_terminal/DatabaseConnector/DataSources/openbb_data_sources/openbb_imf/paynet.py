...
app = FastAPI(title="EquiNova Payment System")
security = HTTPBearer()
...
def create_dodo_payment(amount: float, plan_name: str, user_email: str):
    ...
    dodo_payload = {
        "amount": amount,
        "currency": "USD",
        "description": f"EquiNova {plan_name} Plan",
...
async def home():
    return """
    <html>
        <head><title>EquiNova Payment System</title></head>
        <body style="font-family: Arial; max-width: 800px; margin: 0 auto; padding: 20px;">
            <h1>EquiNova Financial Data Platform</h1>
            <h2>Get Started</h2>
...
async def get_plans():
    ...
    return f"""
    <html>
        <head><title>Payment Plans - EquiNova</title></head>
...
async def payment_success():
    ...
    <html>
        ...
        <body style="font-family: Arial; max-width: 800px; margin: 0 auto; padding: 20px; text-align: center;">
            ...
            <p>Thank you for subscribing to EquiNova. Your account has been activated.</p>
...
async def dashboard(user_id: int = Depends(verify_token)):
    ...
    return HTMLResponse(f"""
    <html>
        <head><title>Dashboard - EquiNova</title></head>
